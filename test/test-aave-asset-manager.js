...
describe("Test AAVE asset manager - running at https://polygonscan.com/block/33313517", function () {
  ...
  beforeEach(async () => {
    ...
    await grantRole(hre, accessManager, "LEVEL1_ROLE", admin);

    // Grant roles to set parameters
    await grantComponentRole(hre, accessManager, jrEtk, "LEVEL2_ROLE", admin);
    await grantComponentRole(hre, accessManager, srEtk, "LEVEL2_ROLE", admin);
  ...
  async function setupAAVEv3PlusVault() {
    ...
    const am = await amContract.deploy(ADDRESSES.usdc, aaveAddress, vault);
    const deployFunction = async function (asset, aaveAddress) {
      return await amContract.deploy(asset, aaveAddress, vault);
    };
  ...
  variants.map((variant) => {
    ...
    it(_tn("Creates an asset manager and does several flow"), async () => {
      ...
      const newBalance = await aToken.balanceOf(jrEtk);
      ...
      expect(await jrEtk.balanceOf(lp)).to.closeTo(newBalance + _A(2000), CENTS);

      // Withdrawal more than ETK liquidity requires deinvestment
      tx = await pool.connect(lp).withdraw(jrEtk, _A(3000));
      receipt = await tx.wait();
      evt = getTransactionEvent(am.interface, receipt, "MoneyDeinvested");
      expect(evt.args.amount).to.be.equal(_A(3000));
      expect(await currency.balanceOf(jrEtk)).to.be.equal(_A(2000)); // jrEtk stays at middle
      expect(await aToken.balanceOf(jrEtk)).to.be.closeTo(newBalance - _A(3000), CENTS);

      // Withdrawal less than ETK liquidity doesn't
      tx = await pool.connect(lp).withdraw(jrEtk, _A(1500));
      receipt = await tx.wait();
      evt = getTransactionEvent(am.interface, receipt, "MoneyDeinvested");
      expect(evt).to.be.equal(null);
      expect(await currency.balanceOf(jrEtk)).to.be.equal(_A(500));
      expect(await aToken.balanceOf(jrEtk)).to.be.closeTo(newBalance - _A(3000), CENTS);
      ...
      expect(await currency.balanceOf(jrEtk)).to.be.equal(_A(2000)); // jrEtk back at middle
      expect(await aToken.balanceOf(jrEtk)).to.be.closeTo(newBalance - _A(4500), CENTS);

      // Setting AM to zero deinvests all
      const preBalance = await aToken.balanceOf(jrEtk);
      ...
      evt = getTransactionEvent(am.interface, receipt, "EarningsRecorded");
      expect(evt.args.earnings).to.be.closeTo(postBalance - preBalance, CENTS);
    });
    ...
    it(_tn("Tests an asset manager shared by two reserves"), async () => {
      ...
      expect(await currency.balanceOf(jrEtk)).to.be.equal(_A(10000));
      expect(await currency.balanceOf(srEtk)).to.be.equal(_A(5000));

      await jrEtk.connect(admin).setAssetManager(am, false);
      await srEtk.connect(guardian).setAssetManager(am, false);
      ...
      expect(await currency.balanceOf(jrEtk)).to.be.equal(_A(10000 - 8000));
      expect(await currency.balanceOf(srEtk)).to.be.equal(_A(5000 - 4000));
      expect(await aToken.balanceOf(jrEtk)).to.be.closeTo(_A(8000), CENTS);
    ...
    it(_tn("Can change the AM"), async function () {
      ...
      await jrEtk.checkpoint();
      expect(await currency.balanceOf(jrEtk)).to.be.equal(_A(160));
      expect(await aToken.balanceOf(jrEtk)).to.be.equal(_A(2000 - 160));
      expect(await jrEtk.totalSupply()).to.be.equal(_A(2000));

      const am2 = await await deployFunction(ADDRESSES.usdc, aaveAddress);
      await jrEtk.connect(admin).setAssetManager(am2, false);
      // am deinvested
      expect(await currency.balanceOf(jrEtk)).to.be.closeTo(_A(2000), _A("0.001"));
      ...
      expect(await currency.balanceOf(jrEtk)).to.be.equal(_A(160));
      expect(await aToken.balanceOf(jrEtk)).to.be.closeTo(_A(2000 - 160), _A("0.001"));
    });
  });
  ...
  it("Tests AAVEv3PlusVaultAssetManager with vault investment", async () => {
    ...
    expect(await vault.balanceOf(jrEtk)).to.be.equal(_A(0));
    ...
    expect(await vault.balanceOf(jrEtk)).to.be.equal(_A(5000));
    expect(await aToken.balanceOf(jrEtk)).to.be.closeTo(_A(3000), CENTS);
    ...
    await vault.lend(lp, _A(3000));
    expect(await vault.totalAssets()).to.be.closeTo(_A(9000), CENTS); // unchanged
    expect(await vault.maxWithdraw(jrEtk)).to.be.closeTo(_A(6000), CENTS); // some not withdrawable
    ...
    expect(await jrEtk.balanceOf(lp)).to.closeTo(_A(10000 - (1000 * 8000) / 9000), CENTS);

    // Earnings too (here 100 as interest)
    await vault.repay(lp, _A(500), _A(100));

    tx = await jrEtk.recordEarnings();
    receipt = await tx.wait();
    evt = getTransactionEvent(am.interface, receipt, "EarningsRecorded");
    expect(evt.args.earnings).to.be.closeTo(_A((100 * 8000) / 9000), CENTS);
    expect(await jrEtk.balanceOf(lp)).to.closeTo(_A(10000 - (900 * 8000) / 9000), CENTS);

    // balance in shares unchanged - in assets changed because of the losses
    const shares = await vault.balanceOf(jrEtk);
    ...
    expect(await jrEtk.balanceOf(lp)).to.closeTo(newBalance + _A(2000) + _A(3000 - (900 * 8000) / 9000), CENTS);
    ...
    expect(evt.args.amount).to.be.equal(_A(3000));
    expect(await currency.balanceOf(jrEtk)).to.be.equal(_A(2000)); // jrEtk stays at middle
    expect(await aToken.balanceOf(jrEtk)).to.be.closeTo(newBalance - _A(1400), CENTS);
    ...
    await jrEtk
      .connect(admin)
      .forwardToAssetManager(
        amContract.interface.encodeFunctionData("aaveToVault", [(await aToken.balanceOf(jrEtk)) - _A(400)])
      );

    let assetsBefore = await vault.totalAssets();
    expect(await vault.maxRedeem(jrEtk)).to.be.closeTo(_A(3586.812), CENTS);
    expect(await aToken.balanceOf(jrEtk)).to.be.closeTo(_A(400), CENTS);
    ...
    expect(await currency.balanceOf(jrEtk)).to.be.equal(_A(2000)); // jrEtk back at middle
    expect(await aToken.balanceOf(jrEtk)).to.be.equal(0);
    expect(await vault.totalAssets()).to.be.closeTo(assetsBefore - _A(1100), CENTS);
    ...
    expect(await vault.previewRedeem(await vault.balanceOf(jrEtk))).to.be.equal(_A(600));

    expect(await currency.balanceOf(jrEtk)).to.be.closeTo(_A(2000) + (assetsInVaultBefore - _A(600)), CENTS);
    evt = getTransactionEvent(am.interface, receipt, "MoneyDeinvested");
    expect(evt.args.amount).to.be.closeTo(assetsInVaultBefore - _A(600), CENTS);
    evt = getTransactionEvent(am.interface, receipt, "EarningsRecorded");
    expect(evt.args.earnings).to.be.closeTo(_A(-600), CENTS);
  });

  it("Tests AAVEv3PlusVaultAssetManager with shared vault", async () => {
    const { amContract, aToken, am, vault } = await setupAAVEv3PlusVault();
    await pool.connect(lp).deposit(jrEtk, _A(10000));
    await currency.connect(lp).transfer(lp2, _A(5000)); // give some money to my friend lp2
    await currency.connect(lp2).approve(pool, MaxUint256);
    ...
    let tx = await pool.connect(lp).withdraw(jrEtk, _A(3500));
    let receipt = await tx.wait();
    let evt = getTransactionEvent(am.interface, receipt, "MoneyDeinvested");
    expect(evt.args.amount).to.be.equal(_A(3500));
    expect(await currency.balanceOf(jrEtk)).to.be.equal(_A(2000));
    expect(await aToken.balanceOf(jrEtk)).to.be.equal(_A(0));
    ...
    tx = await pool.connect(lp2).withdraw(srEtk, _A(4500));
    receipt = await tx.wait();
    evt = getTransactionEvent(am.interface, receipt, "MoneyDeinvested");
    expect(evt.args.amount).to.be.closeTo(_A(4000), CENTS);
